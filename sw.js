 ...
 */
const VERSION = "1.03" // Se incrementa la versión para forzar la actualización del Service Worker
...
if (self instanceof ServiceWorkerGlobalScope) {
  ...
  self.addEventListener("push", (/** @type {PushEvent} */ event) => {
    // Verificar si hay datos en el evento y si el permiso de notificación está concedido
    if (event.data && self.Notification.permission === 'granted') {
      event.waitUntil(muestraNotificacion(event.data));
    }
  })
...
/**
 * @param {PushMessageData} notificacionData
 */
async function muestraNotificacion(notificacionData) {
  if (self instanceof ServiceWorkerGlobalScope) {
    // Parsear los datos de la notificación como JSON
    const data = notificacionData.json();

    // Extraer el título, cuerpo e ícono del objeto JSON
    const title = data.title || 'Nueva Notificación'; // Título por defecto
    const body = data.body || 'Has recibido un nuevo mensaje.'; // Cuerpo por defecto
    const icon = data.icon || 'favicon.ico'; // Ícono por defecto
